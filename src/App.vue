...
type Screen = 'dashboard' | 'scanner' | 'bill' | 'inventory' | 'reports' | 'auth' | 'product-detail' | 'add-product';

const currentScreen = ref<Screen>('dashboard');
const previousScreen = ref<Screen>('dashboard');
...
<template>
  ...
  <div v-else class="min-h-screen bg-surface flex flex-col">
    <!-- Top Bar -->
    <header class="sticky top-0 z-50 bg-surface/80 backdrop-blur-xl border-b border-surface-container-high px-6 py-4 flex justify-between items-center max-w-md mx-auto w-full">
      <div class="flex items-center gap-3">
        <button 
          v-if="currentScreen === 'product-detail' || currentScreen === 'scanner'"
          @click="currentScreen = previousScreen"
          class="p-2 -ml-2 hover:bg-surface-container-high rounded-full transition-colors"
        >
          <ArrowLeft class="w-5 h-5 text-primary" />
        </button>
        <div v-else class="w-10 h-10 rounded-full bg-primary-container flex items-center justify-center overflow-hidden border border-surface-container-highest">
          <img 
      ...
      <div class="flex items-center gap-2">
        <button v-if="currentScreen === 'product-detail'" class="p-2 hover:bg-surface-container-high rounded-full transition-colors text-on-surface-variant">
          <Share2 class="w-5 h-5" />
        </button>
        <button class="p-2 hover:bg-surface-container-high rounded-full transition-colors text-primary relative">
          <Bell class="w-5 h-5" />
          <span class="absolute top-2 right-2 w-2 h-2 bg-error rounded-full border-2 border-surface" />
        </button>
        <button v-if="currentScreen === 'dashboard'" class="p-2 hover:bg-surface-container-high rounded-full transition-colors text-primary">
    ...
    <!-- Main Content -->
    <main class="flex-grow pb-32 max-w-md mx-auto w-full">
      <transition name="fade-slide" mode="out-in">
        <Dashboard v-if="currentScreen === 'dashboard'" @navigate="navigateTo" />
        <Scanner v-else-if="currentScreen === 'scanner'" @navigate="navigateTo" />
        <Bill v-else-if="currentScreen === 'bill'" @navigate="navigateTo" />
        <StockManager v-else-if="currentScreen === 'inventory'" @navigate="navigateTo" />
        <Reports v-else-if="currentScreen === 'reports'" @navigate="navigateTo" />
        <ProductDetail v-else-if="currentScreen === 'product-detail'" @navigate="navigateTo" />
        <AddProduct v-else-if="currentScreen === 'add-product'" @navigate="navigateTo" />
      </transition>
    </main>

    <!-- Bottom Navigation -->
    <nav class="fixed bottom-0 left-0 right-0 w-full z-50 px-4 sm:px-6 pb-[calc(env(safe-area-inset-bottom,0px)+1.5rem)] pt-4 max-w-md mx-auto">
